            ...
            eager: [
              {
                width: 400,
                height: 225,
                effect: "preview:duration_15:max_seg_9:min_seg_dur_1",
              },
            ],
            eager_async: true, // generate in background, don't block upload response (so by the time user hovers over the video, it is already available for preview, epreview is lazy preview, only generates on demand but will use the one generated by eager transformation if there is one. so the previews load faster.)
          },
          (error, result) => {
            if (error) {
              reject(error);
